...
imiss <- is.na(Yna)#simdata$index.miss
C <- simdata$C
...
fit.cv.mrrr <- cv.mrrr(Yna , X, family = family,
                       ...
                       penstr = list(penaltySVD = "rankCon", lambdaSVD = c(1 : 6)),
                       nfold = 5, nlam = 50)
...
a/Iters
mean((X%*%C - X%*%hatc)^2);mean((X%*%Bm - X%*%C )^2);mean((X%*%M.lmc - X%*%C )^2)
mean(( (X%*%hatc)[imiss] - Y[imiss])^2);mean(( (X%*%Bm)[imiss] - Y[imiss])^2);mean(( (X%*%M.lmc)[imiss] - Y[imiss])^2)
